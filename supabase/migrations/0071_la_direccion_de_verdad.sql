-- =============================================================
--  La dirección deja de ser una línea suelta
--
--  `place_order` sólo exigía que la dirección no viniera vacía, así que
--  aceptaba «Dabeiba» —el nombre de la ciudad— y el repartidor se encontraba
--  eso delante. La comprobación va aquí y no en el formulario porque el
--  formulario no es el único camino: quedan el TPV, la API y cualquier
--  importación.
--
--  Se añade `p_address_id` al final de la firma para no mover de sitio ningún
--  parámetro existente. Hay que borrar la función antes de crearla porque
--  añadir un parámetro no reemplaza la anterior: crea una segunda, y entonces
--  PostgREST no sabría a cuál llamar.
-- =============================================================

drop function if exists public.place_order(
  text, jsonb, order_type, payment_method, text, text, text, text, text, text,
  text, integer, text, uuid, integer, text, text, text, uuid, date);
...
-- El escaparate la llama sin sesión: en mesa se pide sin cuenta. La migración
-- 0062 le quita la ejecución a PUBLIC en cuanto se crea, así que hay que
-- volver a concederla a quien la usaba.
